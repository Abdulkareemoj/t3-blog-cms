...
const PostDetail = ({ post }) => {
    const getContentFragment = (index, text, obj, type) => {
        ...
        switch (type) {
            case 'heading-three':
                return (
                    <h3 key={index} className="mb-4 text-xl font-semibold"></h3>
                )
            ...
            case 'heading-four':
                return (
                    <h4 key={index} className="mb-4 text-xl font-semibold"></h4>
                )
    ...
    return (
        <div>
            <div className="mb-8 rounded-lg bg-white pb-12 shadow-lg lg:p-8">
                ...
                <div className="flex-items mb-8 flex w-full">
                    <div>
                    ...
                    {post.content.raw.children.map(
                        (typeObj,
                        (index) => {
                            const children = typeObj.childres.map(
                                (item, itemIndex) =>
                                    getContentFragment(
                                        itemIndex,
                                        item.text,
